...
<xsl:stylesheet
  ...
  exclude-result-prefixes="msxsl helpers">
    ...
    <xsl:template match="/root">
        <p>La seguente segnalazione è stata aggiornata da <xsl:value-of select="Issue/LastUpdateDisplayName" disable-output-escaping="yes" />:</p>
        <table border="0">
            <tr>
                    ...
                    <xsl:if test="count(IssueHistoryChanges/IssueHistory) &gt; 0">
                        <ul>
                            <xsl:for-each select="IssueHistoryChanges/IssueHistory">
                                <li>
                                    <xsl:value-of select="FieldChanged" /> cambiata da "<xsl:value-of select="OldValue" disable-output-escaping="yes"/>" a "<xsl:value-of select="NewValue" disable-output-escaping="yes"/>"
                                </li>
                            </xsl:for-each>
                        </ul>
                    </xsl:if>
