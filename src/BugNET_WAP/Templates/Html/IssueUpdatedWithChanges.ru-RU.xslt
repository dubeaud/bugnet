...
<xsl:stylesheet
  ...
  exclude-result-prefixes="msxsl helpers">
    ...
    <xsl:template match="/root">
        <p>Следующее задание изменено пользователем <xsl:value-of select="Issue/LastUpdateDisplayName" disable-output-escaping="yes" />:</p>
        <table border="0">
            <tr>
                    ...
                    <xsl:if test="count(IssueHistoryChanges/IssueHistory) &gt; 0">
                        <ul>
                            <xsl:for-each select="IssueHistoryChanges/IssueHistory">
                                <li>
                                    <xsl:value-of select="FieldChanged" /> cизменилось с "<xsl:value-of select="OldValue" disable-output-escaping="yes"/>" на "<xsl:value-of select="NewValue" disable-output-escaping="yes"/>"
                                </li>
                            </xsl:for-each>
                        </ul>
                    </xsl:if>
