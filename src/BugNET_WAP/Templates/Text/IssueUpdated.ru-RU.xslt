...
<xsl:stylesheet
  ...
  <xsl:template match="/root">
    <xsl:text>Следующее задание обновлено пользователем </xsl:text> <xsl:value-of select="Issue/LastUpdateDisplayName" disable-output-escaping="yes" /><xsl:text>:</xsl:text>
    <xsl:text>&#10;</xsl:text>
    <xsl:text>&#10;</xsl:text>
