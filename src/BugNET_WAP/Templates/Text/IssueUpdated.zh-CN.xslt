...
<xsl:stylesheet
  ...
  <xsl:template match="/root">
    <xsl:text>如下问题被 </xsl:text> <xsl:value-of select="Issue/LastUpdateDisplayName" disable-output-escaping="yes" /><xsl:text>更新:</xsl:text>
    <xsl:text>&#10;</xsl:text>
    <xsl:text>&#10;</xsl:text>
